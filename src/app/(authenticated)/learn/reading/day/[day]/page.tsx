...
import { ReadingViewer, ReadingItem } from "@/components/learn/ReadingViewer";
...
export default function ReadingDayPage({
  ...
  useEffect(() => {
    ...
    async function loadReading() {
      try {
        const res = await fetch(`/api/learn/reading/${studyDay}`);
        ...
        if (!isMounted) return;
        setErrorMessage(data?.error?.message || "Không thể tải bài đọc.");
        setIsLoading(false);
      } catch {
        if (!isMounted) return;
  ...
  if (contentState === "pending" || items.length === 0) {
    return <ContentPending message="Nội dung bài đọc ngày này chưa được chuẩn bị." />;
  }
