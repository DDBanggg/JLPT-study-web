...
describe("shared Test Engine", () => {
  ...
  it("loads and scores the Chapter 3 Weekly Test", async () => {
    const roadmap = await loadProgramRoadmap();
    if (roadmap.state !== "available") throw new Error("Roadmap missing");
    const location = findTestLocation(roadmap.data, "weekly-03");
    if (!location) throw new Error("Chapter 3 Weekly Test missing from roadmap");
    const loaded = await loadTestContent(location);
    if (loaded.state !== "available") throw new Error("Chapter 3 Weekly content missing");

    expect(loaded.data.coverage).toEqual({ from_day: 26, to_day: 31 });
    expect(loaded.data.sections.map(({ id, questions }) => [id, questions.length])).toEqual([
      ["language", 30],
      ["reading", 12],
    ]);
